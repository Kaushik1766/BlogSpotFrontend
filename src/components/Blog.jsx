...
function Buttons({ author, id, reRender }) {
    function deletePost(id) {
        if (window.confirm('Are you sure you want to delete the post?')) {
            axios.post(import.meta.env.VITE_BACKEND + "/del", { id: id }).then(
                () => { reRender() }
            )
        }
    }
    ...
    if (username == 'Admin') {
        return <div className="">
            <button type="button" className="btn btn-primary align-self-end mx-2" onClick={() => { window.location.href = `#/edit/${id}` }} >Edit</button>
